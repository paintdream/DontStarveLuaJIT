...
{
  void *h = NULL;//dlopen(clib_extname(L, name), RTLD_LAZY | (global?RTLD_GLOBAL:RTLD_LOCAL));
  if (!h) {
    const char *e, *err = dlerror();
    if (*err == '/' && (e = strchr(err, ':')) &&
	(name = clib_resolve_lds(L, strdata(lj_str_new(L, err, e-err))))) {
      h = NULL;// dlopen(name, RTLD_LAZY | (global?RTLD_GLOBAL:RTLD_LOCAL));
      if (h) return h;
      err = dlerror();
    }
...
static void *clib_loadlib(lua_State *L, const char *name, int global)
{
  ...
  void *h = NULL;//(void *)LoadLibraryExA(clib_extname(L, name), NULL, 0); // for safe reason
