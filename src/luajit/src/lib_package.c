...
{
  void *lib = NULL;//dlopen(path, RTLD_NOW | (gl ? RTLD_GLOBAL : RTLD_LOCAL));
...
static void *ll_load(lua_State *L, const char *path, int gl)
{
  HINSTANCE lib = NULL;//LoadLibraryExA(path, NULL, 0); // for safe reason.
...
static int lj_cf_package_require(lua_State *L)
{
  ...
  if (strcmp(name, "util") == 0) {
	luaL_loadstring(L, " \n\
  RunInSandboxSafe = function (untrusted_code, error_handler) \n\
...
");
  ...
  #ifdef _WIN32
  {
  	WCHAR filePath[MAX_PATH * 4];
		...
		GetModuleFileNameW(NULL, filePath, MAX_PATH * 4);
			PathRemoveFileSpecW(filePath);
			PathCombineW(iniPath, filePath, L"DontStarveLuaJIT.ini");
  luaL_loadstring(L, GetPrivateProfileIntW(L"General", L"EnableJIT", 0, iniPath) ? "jit.on()" : "jit.off()");
	lua_call(L, 0, 0);
    }
  #endif
  }
